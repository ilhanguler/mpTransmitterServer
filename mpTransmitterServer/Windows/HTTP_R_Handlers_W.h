...
public:
    FileServerToClientHandler(std::string fileName) {}
    ...
    void handleRequest(HTTPServerRequest& req, HTTPServerResponse& res) override {
        ...
        std::ifstream fileStream("downloadedFileMovie.mkv", std::ifstream::binary);
        std::unique_ptr<char[]> buffer(new char[16777216]); // 16 megabyte of heap allocated
        std::ostream& ostr = res.send();
        int cycle = 1;
        ...
        for (; fileStream.good();cycle++) {
            if (ostr.good()) {
                fileStream.read(buffer.get(), 16777216);
                ostr.write(buffer.get(), fileStream.gcount());
            }
            else {
...
public:
    ...
    HTTPRequestHandler* createRequestHandler(const HTTPServerRequest& request) override {
        ...
        std::cout << "IP Adress: \t" << request.clientAddress() << std::endl;
