...
namespace hr {
	...
	// Parse the input to generate DOM tree. Returns element pointer.
	dom::Element* htmlToDOM(std::string_view html) {
		...
		for (; std::string::npos != (cur = html.find("<", beg)); ) {
			...
			if (cutstr[1] == '/') {
				...
				{std::cout << "CLOSED:\t" << closedTag++ << std::endl; }
#endif // DEBUG  
			...
			if (std::string::npos != contains(voidElems, tokens.front())) { // Void tags
				...
				// cur = html.find("</", beg); 
				// ^This could be enough. I did not test side effects.^
...
#if 0 // DEBUG
				{std::cout << "OPENED:\t" << openedTag++ << std::endl; }
#endif // DEBUG  
